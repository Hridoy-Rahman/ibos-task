          ...
          {user?.email ? (
            <div className="relative">
              <button onClick={handleUserClick} className="flex items-center focus:outline-none">
                {user?.photoURL ? (
                  <img className="h-8 w-8 rounded-full" src={user.photoURL} alt={user.email} />
                ) : (
                  <img
              ...
              {showLogout && (
                <div className="absolute right-0 mt-2 bg-white shadow-lg rounded py-2">
                  <button onClick={handleLogout} className="w-full text-[#1E99F5] px-2">
                    Logout
                  </button>
                </div>
              )}
            ...
            <Link to="/login" className="text-black font-semibold hover:text-gray-500">
              Login
            </Link>
          )}
        </div>
